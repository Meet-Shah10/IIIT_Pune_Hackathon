...
import {
  ...
  ChevronDown, Bell, PanelLeftClose, PanelLeftOpen, X, Trash2, LogOut
} from 'lucide-react'
      ...
      >
        ...
        <div className="px-4 mt-8 flex-1 overflow-y-auto no-scrollbar">
          <button 
            ...
            className="w-full flex items-center justify-between text-xs font-semibold text-zinc-500 px-3 mb-2 hover:text-zinc-700 transition-colors"
          >
            <span>Sessions</span>
            <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isSessionsOpen ? '' : '-rotate-90'}`} />
          </button>
        ...
        <div className="mt-auto">
          <div className="px-4 py-3 border-t border-zinc-200 flex items-center justify-between hover:bg-zinc-200/30 transition-colors">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-zinc-800 flex items-center justify-center text-white text-xs font-bold">
                {user?.name?.charAt(0)?.toUpperCase() || 'U'}
              </div>
              <span className="text-xs font-medium text-zinc-700 truncate max-w-[100px]">{user?.name || 'User'}</span>
            </div>
